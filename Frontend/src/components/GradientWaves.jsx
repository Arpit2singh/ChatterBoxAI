...
const GradientWaves = ({
  ...
  useEffect(() => {
    ...
    const renderer = new Renderer({
      ...
      dpr: 1 // Force DPR to 1 (disables retina scaling) to save 4x fill rate on heavy raymarching shaders
    });
    ...
    const setSize = () => {
      ...
      // Scale render buffer for 60fps performance while keeping canvas full viewport
      const renderScale = Math.min(1, Math.max(0.6, 1280 / Math.max(w, h)));
      const renderW = Math.max(1, Math.floor(w * renderScale));
      const renderH = Math.max(1, Math.floor(h * renderScale));
      renderer.setSize(renderW, renderH);
      // Explicitly lock CSS dimensions to full parent container
      canvas.style.width = '100%';
      canvas.style.height = '100%';
